...
import { getClassInformation,addClass,updateClass,deleteClass } from '@/api/system/sys_class.js'
...
export default {
  ...
  methods:{
    ...
    handleSearch(){
      ...
      getClassInformation({id:this.materialQueryForm.idLike}).then((res)=>{
        this.setData(res);
      })
    },
    ...
    submitForm(){
      if(this.form){
        if(this.form.id){
          ...
          addClass({data:this.form,token:getJwtToken()}).then((res) => {
            if(res.code !== 0){
